...
class SearchControlsAndViewsTests(unittest.TestCase):
    def test_search_controls_share_one_row_without_native_source_or_limit_selects(self) -> None:
        ...
        self.assertIn('data-source="word"', HTML)
        self.assertIn('data-source="epub"', HTML)
        self.assertNotIn('>Word / EPUB</button>', HTML)
    ...
    def test_import_page_progressively_reveals_local_mineru_mode(self) -> None:
        ...
        self.assertIn('accept=".pdf,.docx,.epub,.mefinder.zip"', HTML)
        self.assertIn("['.pdf', '.docx', '.epub']", HTML)
    ...
    def test_mineru_settings_support_independent_token_accounts(self) -> None:
        ...
        self.assertIn("fetch('/api/mineru-local/component'", HTML)
    ...
    def test_backup_export_import_is_wired(self) -> None:
        ...
        self.assertIn("function chooseDesktopExportDirectory()", HTML)
        self.assertIn("desktopShell !== 'macos' && desktopShell !== 'win32'", HTML)
