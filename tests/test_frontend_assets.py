...
class FrontendAssetAssemblyTests(unittest.TestCase):
    ...
    def test_large_domain_modules_keep_bounded_global_command_surfaces(self):
        ...
        budgets = {
            # 作品组归组命令：一键归组 / 同名合并建议 / 内嵌加入 /
            # 「加入作品组」下拉的加入与新建（净 40）。
            "static/js/30-library.js": 40,
...
class FrontendAssetBaselineTests(unittest.TestCase):
    ...
    # 作品组：选择栏「加入作品组 ▾」下拉（已有组一步加入 + 新建 + 管理），并去掉并排的归组/设置两按钮。
    BASELINE_SHA256 = (
        "7d3a25314a685fe76bb3cf89ca7f75384fbb58a40306e9550450e6404fcab1b4"
    )
    BASELINE_BYTES = 986406
