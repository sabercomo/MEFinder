...
- **batch64-vs-16 前置实验已执行**(更正:模型在真实 app 运行时目录 `~/Library/Application Support/MEFinder/runtime/components/text-alignment/models`,先前误判缺失)。结果见 [batch 报告](../../reports/batch-size-compare-2026-09-13.md):完整对齐链接**结构完全一致**(967=967 无翻转,分数差 ≤1e-4)、逐向量余弦≈1(max_abs 3.3e-4,96.5% 逐位一致,差异仅中/长文本)、**verdict=review-vectors-changed-links-stable**。数值近乎等价,但**不得凭单对"差异很小"改默认**:须(1)多对/边界样本泛化、(2)决定是否 bump `EMBEDDING_RUNTIME_VERSION`(向量非逐位相同,缓存会混用)、(3)确认 ~3e-4 漂移不翻转下游定位/复用。**产品默认维持 batch 64。** 对齐侧外层 tracemalloc 精确重测现也解锁(模型在),留作后续。
