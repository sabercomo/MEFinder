...
- **batch64-vs-16 前置实验已执行并按审计修正口径**(更正:模型在真实 app 运行时目录,先前误判缺失;且改用**实际逐段向量**——生产 `embed_text_sequences` 先去重 3948→3837 再推理回填,旧的原文直嵌入 batch 分组不同)。结果见 [batch 报告](../../reports/batch-size-compare-2026-09-13.md):链接**结构完全一致**(967=967 无翻转),但 confidence/cost 有极小非零差(≤1.02e-4,**不忽略**),向量 96.8% 逐位一致、`min_cosine 0.99999982`、短文本逐位一致——故**非"输出完全一致"**,**verdict=manual-review-required**(工具不凭自设阈值判"safe")。采纳仍须(1)多对/边界样本泛化、(2)质量门槛、(3)`EMBEDDING_RUNTIME_VERSION` 缓存版本决策。**产品默认维持 batch 64,不据"差异很小"采纳。** 对齐侧外层 tracemalloc 精确重测现也解锁(模型在)。
