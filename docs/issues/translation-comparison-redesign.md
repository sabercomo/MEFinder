...
## 2026-09-20 — 阅读会话重构复审：两处异步竞态

事实：对 `e58562c` / `7682c14` 执行真实 JS 函数并控制请求完成顺序，复现两处问题：

1. 任务结束处理等待 A 的对齐目标列表时切到 B；随后旧处理继续调用 `loadWorkContext(A)`，递增共享请求序号，B 的有效响应被丢弃，A 又因源文献不匹配被丢弃，B 最终没有作品上下文。
2. `loadPosition(G)` 在途时关闭阅读器并交回新位置；旧 GET 随后返回或失败，覆盖刚交回的位置，使「继续阅读」回退或消失。

修复：任务处理在两次异步读取之间重新检查阅读器是否仍打开且源文献未切换；位置首次读取的响应应用前检查缓存是否已由关闭交接或另一读取填充，旧成功/失败响应均不得覆盖它。保持无 400ms 定时重查的交接方式。

验证：新增 `test_finished_job_does_not_invalidate_the_next_books_context`（切书、关闭）及 `test_close_handoff_wins_over_an_inflight_position_read`（旧读取成功、失败与正常首次读取），均先失败后通过。94 项相关检查、Ruff 和两个 JS 文件语法检查通过；隔离工作目录全量 2467 项通过（23 跳过）。本次未重建安装包或改动 MinerU。

并行边界：从 MinerU 适配提交 `c4acc61` 建立独立 worktree，仅修改阅读器/作品页、其测试、前端指纹及本条文档；不纳入共享工作区桌面启动与内存测量 WIP。
