...
## 2026-09-12 复核与修正：原验收有假阳性

**事实**：上文旧原型仅检查 reader 页面 `document.body`，没有提供 `reader_options` 或触发 `pywebviewready`，正文可能仍处于等待状态；定位仅查“页／第”字样；退出在 `terminate` 前将 `exit.ok` 写为 true，之后的后台退出错误没有写回报告。上述证据不足以支持原来的“五项通过”与体验成本结论。

本次修复后证据见 [隐藏原型报告](../../reports/native-host-acceptance-2026-09-12.json)：

- 主窗口实际搜索固定合成原句，核对返回字符区间与详情中的精确高亮。
- WKWebView 提供最小阅读 bridge，执行产品 `reader-window.js` 的初始化；核对正文加载完成、文献 ID、页索引、锚点和高亮原句。
- 两个阅读窗打开不同文献，原生坐标移动后第一窗定位不变；通过产品关闭通知关闭第二窗，重开后核对锚点。
- 主窗通过 WK 消息请求原生 PDF 窗口加载合成三页 PDF，跳至第 2 页并核对该页正文。此项验证原型 bridge／PDFKit 路径，**尚未接入产品主界面“打开原文”的完整入口**。
- 子进程先退出 AppKit 事件循环，再停止后台并收集退出码；外层进程确认宿主实际结束后才发布最终报告。本次后台、宿主退出码均为 0。
- 新回归测试拒绝空阅读壳、错误文献/页/锚点、缺少高亮、后台非零退出/超时、宿主退出但没有报告等情况。

原型现在默认全部隐藏运行，使用临时合成库，不读用户库或模型；后台独立进程的真实模型验收改为复制单个模型缓存至临时目录，避免向用户缓存写验证回执。

**当前结论**：基础技术路径可行；本次未验证可见拖动手感、菜单、全屏、原生 PDF 入口、Swift 打包、更新或双平台发布。因此不能将任务 6 标记为完整产品验收，也不能凭代码行数断言“收益不抵成本”。继续维持当前产品宿主是范围控制决策；原生迁移仍应由具体需求和可见交互实测决定。
