...
## 2026-09-14：第三轮复审与直接修复

事实：`da9983e` 已修复探测的生命周期归属、启动失败后的临时目录清理、隔离缓存写穿安装回执以及运行时错误分类；其 CI run `34830924529` 已核验成功。继续复审时通过回归测试发现：排队请求可清除正在关闭的任务的取消信号，模型未安装仍被包装成解析失败，强制终止返回时尚未回收 worker，临时目录删除失败被静默忽略。

本次将取消信号重置移入 mutation 锁、置于 durable operation 准入前，并在准入后构造 runner；排队请求不得影响当前任务的取消。模型缺失也归为组件不可用，HTTP 契约见 [v0.5.4-alignment-jobs.md](../contracts/v0.5.4-alignment-jobs.md)。发送强制终止信号后必须等待子进程退出；临时文件清理失败明确传播错误。

新增真实子进程的运行时关闭测试，分别覆盖探测与计算阶段：关闭成功返回前进程已回收、临时文件已清理、后台任务返回取消且数据库没有新完成成果。完整复现与门禁见 [第三轮复审报告](../../reports/alignment-compute-2a-final-review-2026-09-14.md)。本轮不改变算法、阈值、batch64、缓存语义或分发方式。
