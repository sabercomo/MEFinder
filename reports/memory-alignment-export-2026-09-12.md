...
- **正式三轮四场景服务器基准——已跑,`--compare` 已跑通**(2026-09-13,机器安静;见 [四场景报告](performance-real-4scenario-2026-09-13.md)):当前修复后代码四场景全 12 运行 0 错误、**0 次 503**、identity_mismatches=0;对齐期间重叠搜索全 200,峰值 RSS 1279–1314 MiB。**注意**:通过的 `--compare` 是**同版本重复测量**(两份 JSON 均 `109e089`),只证明工具+当前版本能完成完整四场景测量;旧记录 28.2% 与本轮 0-503 分别陈述,**降幅未由该同版本比较证明**,须用同工具跑旧/新版产品代码对照。
