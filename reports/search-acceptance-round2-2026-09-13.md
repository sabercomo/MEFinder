...
- **服务器级基准:normal(搜索)场景已跑**(见"附",逐查询 p50 坐实"社会"联合请求 ~2.4 s)。**完整三轮四场景已跑,`--compare` 已跑通**(2026-09-13,模型就位,见 [四场景报告](performance-real-4scenario-2026-09-13.md)):四场景全 0 错误、0 次 503、对齐重叠搜索全 200。注意通过的比较是**同版本重复测量**(非优化前后);旧 28.2% 与本轮 0-503 分别陈述,降幅须用同工具跑旧/新版产品代码才能证明。
