...
2026-09-12:高频短查询("社会""社"等 <3 字、全库无过滤)从 ~0.4s(暖)/ ~2.3s(冷)降到 ~18–28ms,**完整结果逐字节不变**(12 条查询结果摘要 SHA-256 一致 `244f91db…`)。scoped、FTS(≥3 字)、繁简稀有词路径按设计不变。未新增索引、未改 schema,未触碰候选预算/评分/去重/total 语义/上下文/引文/页码/字符锚点。
