...
- [x] 滚动页码条：随当前页更新，严格按 `page_source_type` 映射文案显示，未校准绝不显示裸"第 N 页"。
- [x] 点击页码 → 调 `build_citation_formats` 复制中文脚注 / GB/T 7714；未校准时显示"页码未验证"并拒绝带页码引文。
- [x] 跨页选择文字 → 自动生成页码范围（仍走 `citations.py`）。
- [x] 深链：`/reader?source=&page=&off=&h=&q=`，按恢复阶梯执行；关闭重开回到原位置。
- [x] 长文档滚动性能复测（上百页、快速拖动滚动条）。
