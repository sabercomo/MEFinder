...
| **引文核对 / 主题检索（MCP）** | 让支持 MCP 的 AI 助手只读访问文献库：批量核对引文是否逐字命中、逐字比对错引差异、按主题召回相关段落、读取版权页与题录缺口。 |
...
1. 在文献库选中要归到一起的版本，点击 **设置作品组**，新建或加入一个作品组——把同一部作品的原文、译本、不同版本放进同一组（作品组不是文件夹）。
2. 在该组的 **生成双栏对照** 区域选择两个版本，点击 **生成对照**。PDF 和 EPUB 可以混着选，不必先指定基准版本。
3. 打开双栏对照阅读：滚动左栏时右栏自动跟随并高亮对应文字，也可以关闭 **自动跟随** 单独翻阅右栏；在结构化阅读器里选中文字，可以定位到另一版本的对应位置。
...
### MCP 文献核对（0.5.1，可选只读集成）

0.5.1 提供九个本地只读 MCP 工具，让支持 MCP 的 AI 助手直接读你的文献库，覆盖定位、跨译本对照、核对、检索和题录：
...
- **译本 / 原文对照**：`find_parallel_passages` 接收一句中文或任一版本的句子，从作品组已生成的对齐中返回英文、原文或其他译本的对应文本；可按目标语言或具体版本收窄，不现场机器翻译。
- **题录**：`read_bibliographic_pages` 读取版权页 / CIP 页等候选文本供提取题录，`read_bibliographic_metadata` 列出现有题录字段并标注已填 / 存疑 / 缺失。

这些工具只读取已导入文献和已保存的版本对齐，不提供问答、总结或翻译等生成功能。Windows 安装版、绿色版和 macOS 发布包都包含独立 `MEFinderMCP` sidecar，不用装 Python；源码模式也可单独接入。桌面窗口无需保持开启，MCP 进程本身不联网。
