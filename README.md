...
我是一名在读文科硕士生。以前写几篇小论文时，每到定稿前，我都要把正文里的引文一条条重新核对：原文有没有抄错，出处有没有写对，页码是否准确，对上下文的理解有没有偏差。写作过程中，我也常常记得某个观点或大致表述，却一时想不起原句究竟怎么写、藏在哪份文献里。

现在写大论文，文献更多了。我常常要在多个 PDF 之间来回切换，反复打开、搜索、翻页和核对。就是在这些反复出现的麻烦里，我开始想：如果有一个工具，能把自己的文献放在一起，既能凭记得的片段找回原句，也能根据论文中的引文找到对应出处、上下文和页码，会不会省下很多时间？

我找了一圈，至少没有发现真正贴合这种需求的软件，于是决定自己动手做一个。MEFinder 就这样开始了。

把完整原句、一个片段，甚至带着少量错字和漏字的文字交给它，它会在你的本地文献库里找回原文，带你回到上下文和原始页面，并整理出便于继续核对、复制的出处。

它不替你“猜”引文来自哪里，也不会给你一个听起来像真的答案。搜索、索引、页码映射和大部分元数据处理默认都在本机完成；只有扫描件、乱码文本层或复杂版面确实需要解析时，才由你决定是否交给 MinerU 或其他视觉服务。

这也是我第一次完整开发一个软件项目。一路上磕磕绊绊，很多东西都是边做边学，MEFinder 也还在继续完善。如果它没找到你确定存在的句子，或者页码、题录处理得不对，欢迎带着样例提交 Issue。感谢大家愿意试用，不足之处还请多多包涵。

如果 MEFinder 恰好帮你少翻了几次 PDF、少花了一些核对引文的时间，欢迎顺手点个 Star 🌟。对第一次做开源项目的我来说，你的支持会是很大的鼓励，也会让我更有动力继续把它做好。
