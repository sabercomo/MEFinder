...
这是一个本地运行的文献学检索工具，用于在 Word 文档和 PDF 文献中定位引文。

第一版不使用向量数据库、Embedding、LLM API 或在线服务。
...
data/index.json      # 可读导出和迁移备份
```

SQLite 数据库保存来源、卷次、文献、段落、页码映射和检索字段。JSON 只作为导出与回退副本保留，桌面应用不会加载它。
...
只处理前 N 本 PDF 时才使用 `--pdf-limit N`。当前 `config/pdf_imports.json` 登记了 `corpus/raw_pdf/` 中的 10 本 PDF；原生文本 PDF 会直接入库，损坏文本层会登记并标记为需要 MinerU。
...
目前包含 20 条样例，覆盖完整原句、缺少标点、标点错误、多余空格、少量错字、少量漏字、截取片段和重复表述。
...
- 第 1 卷是 DOCX，页码来自 DOCX 分节结构推断，尚未按 `PAGE_NUMBER_STRATEGY.md` 完成人工抽样验证。
- 第 2-10 卷是旧版二进制 DOC。第一版使用 OLE/UTF-16 文本流进行本地抽取，页码只能显示目录页码范围，不能视为段落级精确原书页码。
- 标题识别优先使用目录页码范围和正文标题候选，复杂多行标题仍可能需要后续校验。
- 搜索结果会标明页码来源；未验证页码不会伪装成精确页码。
- 当前数据库包含 10 个 PDF 来源，其中 7 个原生文本或 MinerU 结构化 PDF 可以搜索；《批判理论》已通过两个 MinerU 分段接入 653 条正文记录。
- 《伦理学简史》《消费社会》《追寻美德：道德理论研究》当前被检测为损坏文本层，已登记但暂不把乱码写入正文索引；通过应用重新导入后会自动提交 MinerU。
- `Critique of Forms of Life` 已为正文页配置人工分段页码映射：PDF 物理页 22 对应引用页码 1；未校准的 PDF 仍会明确显示未校准。
