...
从仓库的 **Releases** 页面下载 `MEFinder-v0.1.4-windows-portable.zip`，完整解压后双击 `文献原句定位器.exe` 即可运行，无需安装 Python。
...
MinerU 解析失败时默认停止任务并提示用户自行切换。也可以在设置中打开“MinerU 失败后自动切换”；开关会立即保存，并自动使用列表中的首个已启用且配置完整的备用接口。界面会明确提示该路径可能产生模型调用费用。失败任务也可直接在导入队列中手动改用备用接口重试，无需重新上传 PDF。
