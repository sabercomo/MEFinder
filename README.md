...
| **译本 / 版本对照** | 把同一作品的原文、译本或不同版本归入作品组，选两个版本自动逐段对齐，双栏对照阅读并在版本间定位对应文字；对齐在本机完成，PDF 与 EPUB 均可参与。 |
...
2. 在该组的 **生成双栏对照** 区域选择两个版本，点击 **生成对照**。支持 PDF 与 EPUB 任意组合，不必先指定基准版本。
3. 打开双栏对照阅读：滚动左栏时右栏自动跟随并高亮对应文字，也可以关闭 **自动跟随** 单独翻阅右栏；在结构化阅读器里选中文字，可以定位到另一版本的对应位置。

对齐结合跨语言语义和章节结构，支持一句对应多句等情况，全部在本机计算，不上传文献内容；首次使用会按需联网下载本地跨语言语义模型。当前一次只对照两个版本，不提供三栏对照；自动对齐用于辅助定位，不能代替对原文和译文的人工核对。
