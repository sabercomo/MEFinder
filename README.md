...
### MCP 文献核对（0.5.0，可选只读集成）

0.5.0 提供八个本地只读 MCP 工具，让支持 MCP 的 AI 助手直接读你的文献库，覆盖定位、核对、检索和题录四类：
...
这些工具只在已导入文献中定位并返回原文，不提供问答、总结或翻译等生成功能。Windows 安装版、绿色版和 macOS 发布包都包含独立 `MEFinderMCP` sidecar，不用装 Python；源码模式也可单独接入。桌面窗口无需保持开启，MCP 进程本身不联网。
