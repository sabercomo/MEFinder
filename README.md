...
| **PDF / Word / EPUB / 扫描件** | Word、EPUB 和文本型 PDF 在本机解析；EPUB 保留出版方 page-list/pagebreak 页码；扫描版、乱码文本层与复杂布局 PDF 可按需接入 MinerU 或视觉模型。 |
...
| **译本 / 版本对照** | 把同一作品的原文、译本或不同版本归入作品组，选两个版本自动逐段对齐，双栏对照阅读并在版本间定位对应文字；对齐在本机完成，支持 PDF 和 EPUB 格式。 |
...
原生文本 PDF 优先由 PyMuPDF 读取，Word 和 EPUB 文献走本地解析。EPUB 按 OPF spine 的阅读顺序提取正文，只采用出版方提供的 page-list/pagebreak 页码。只有扫描版、乱码文本层或复杂布局 PDF 需要 OCR / 视觉解析；只有你主动使用在线解析时，待处理的页面才会被上传。
...
2. 打开左侧栏的 **译本对照**，在作品的版本清单里勾选两个版本：尚未对齐时点 **生成对齐**，已有对齐时点 **对照阅读**。版本改名、增减、换基准和删除作品都在 **管理版本** 里。
3. 阅读器里可以随时 **添加对照版本** 或关闭对照；滚动左栏时右栏跟随并高亮对应段落，点一段原文可查看它的对应段；算法置信度偏低的段落旁有 **!**，可以人工勾选对应的译文段（支持一对多）、标记译本无对应或暂不处理。检索结果和文献库也进入同一个阅读器。

在 **设置 → 译本对齐 → 对齐算法** 可选 **Bertalign + LaBSE（实验）**。首次安装会在独立环境安装计算依赖并下载约 1.8 GB 的固定版本 LaBSE；安装后计算离线完成。该选择同时作用于生成、作品总览和阅读器；默认仍为现有算法，两种算法的结果分别保存。Bertalign 的相似度分数不代表正确率，当前不能据此断言比默认算法更准。验收范围见 [产品接入报告](reports/bertalign-product-integration-2026-09-21.md)。

对齐组件卸载后，已有对齐仍可只读阅读，生成新对齐需要重新安装。页面上的「已匹配段落」只表示算法找到对应的段落比例，不代表对应一定准确。

对齐会综合跨语言语义和章节结构，一句对应多句的情况也能处理，全部在本机计算，不上传文献内容；首次使用会按需联网下载本地跨语言语义模型。当前一次只对照两个版本，不提供三栏对照；自动对齐用于辅助定位，不能代替对原文和译文的人工核对。
...
0.5.1 提供十三个 MCP 工具，让支持 MCP 的 AI 助手直接访问你的文献库，覆盖定位、跨译本对照、核对、检索、题录，以及人工对齐修正：
