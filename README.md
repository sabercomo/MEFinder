...
MEFinder 是给论文写作、文献阅读和资料核对用的。输入完整原句、片段，甚至带少量错漏的文字，就能从本地文献库中找回对应文献，继续看上下文、定位原页、校准引用页码，并复制规范出处。

它不依赖大模型“猜出处”。搜索、索引、页码映射和大部分元数据处理默认都在本机完成；只有在你主动选择后，扫描件、乱码文本层和复杂版面材料才会交给 MinerU 或其他视觉解析服务。
