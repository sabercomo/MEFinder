...
(function (global) {  // module: 60-settings.js
  ...
  function showSettingsCategory(sectionId) {
    ...
    if (sectionId === 'text-alignment-settings') {
      global.loadAlignmentModelComponent();
      global.loadAlignmentRuntime();
    }
  }
  ...
  function applyPreferencesData(data, requestedThemeRevision) {
    ...
    settingsStore.currentAlignmentEmbeddingModel = data.alignment_embedding_model_id === 'multilingual-e5-large'
      ...
      : 'minilm-l12-v2';
    settingsStore.scriptFoldingEnabled = data.script_folding !== false;
    settingsStore.scriptFoldingAvailable = data.script_folding_available === true;
    renderScriptFolding();
    ...
    global.renderAlignmentEmbeddingModel();
    global.renderAlignmentModelComponent(settingsStore.alignmentModelComponent);
  ...
  function renderScriptFolding() {
    var input = document.getElementById('script-folding-enabled');
    if (input) {
  ...
  async function setScriptFolding(enabled) {
    if (settingsStore.scriptFoldingSaving || settingsStore.preferencesLoadPromise || !settingsStore.scriptFoldingAvailable) {
      renderScriptFolding();
      return;
    }
    var previous = settingsStore.scriptFoldingEnabled;
    settingsStore.scriptFoldingEnabled = enabled === true;
    ...
    try {
      ...
      var data = await resp.json();
      if (!resp.ok || data.error) throw new Error(data.error || '保存失败');
      settingsStore.scriptFoldingEnabled = data.script_folding === true;
    } catch (e) {
      settingsStore.scriptFoldingEnabled = previous;
      showToast('繁简统一检索保存失败：' + e.message);
    } finally {
      settingsStore.scriptFoldingSaving = false;
      renderScriptFolding();
    }
  }
  ...
  async function loadPreferences() {
    ...
    settingsStore.preferencesLoadPromise = (async function() {
      try {
        ...
        if (desktopShell === 'win32') {
          global.loadUpdateStatus().then(function(state) {
  ...
  global.deleteCurrentCustomTheme = deleteCurrentCustomTheme;
  global.exportCurrentTheme = exportCurrentTheme;
  global.triggerThemeImport = triggerThemeImport;
  global.onThemeImportFile = onThemeImportFile;
  global.showSettingsCategory = showSettingsCategory;
  global.ensureVisibleSettingsCategory = ensureVisibleSettingsCategory;
  global.setSettingsSection = setSettingsSection;
  global.openVisionSettings = openVisionSettings;
  global.setCitationStyleEnabled = setCitationStyleEnabled;
  ...
  global.renderScanDirectories = renderScanDirectories;
