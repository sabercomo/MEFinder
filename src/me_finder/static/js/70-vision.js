...
(function (global) {  // module: 70-vision.js
  ...
  global.loadMineruConfig = loadMineruConfig;
  ...
  global.bindMineruAccountDialogDismissal = bindMineruAccountDialogDismissal;
  global.visionBaseActiveIndex = visionBaseActiveIndex;
  global.visionBaseShowAll = visionBaseShowAll;
  global.openVisionBasePop = openVisionBasePop;
  global.closeVisionBasePop = closeVisionBasePop;
  global.toggleVisionBaseList = toggleVisionBaseList;
  global.pickVisionBase = pickVisionBase;
  global.visionBaseKeydown = visionBaseKeydown;
  global.visionModelActiveIndex = visionModelActiveIndex;
  global.openVisionModelPop = openVisionModelPop;
  global.closeVisionModelPop = closeVisionModelPop;
  global.pickVisionModel = pickVisionModel;
  global.visionModelKeydown = visionModelKeydown;
  global.fetchVisionModels = fetchVisionModels;
  global.maybeAutoFetchVisionModels = maybeAutoFetchVisionModels;
  global.configuredVisionProviders = configuredVisionProviders;
  global.selectedImportRecoveryProviderId = selectedImportRecoveryProviderId;
  global.syncImportRecoveryProvider = syncImportRecoveryProvider;
  ...
  global.positionImportVisionMenu = positionImportVisionMenu;
  global.loadVisionProviders = loadVisionProviders;
  global.updateVisionEditorHead = updateVisionEditorHead;
  global.autoFillVisionName = autoFillVisionName;
  ...
  // node 纯逻辑单测（tests/test_frontend_pure_logic.py）访问的内部函数：仅在 node
  // 下（typeof module !== "undefined"）挂到 global，浏览器 window 不暴露，隔离不破。
  if (typeof module !== "undefined" && module.exports) {
    global.hideVisionPop = hideVisionPop;
    global.revealVisionPop = revealVisionPop;
    global.managedMineruErrorText = managedMineruErrorText;
    global.managedMineruSummaryLabel = managedMineruSummaryLabel;
    global.managedMineruTransferSummary = managedMineruTransferSummary;
    global.renderManagedMineru = renderManagedMineru;
    global.visionModelFlat = visionModelFlat;
    global.visionNameAutoValue = visionNameAutoValue;
    // 测试钩子：visionNameAutoValue 是本模块私有可变状态，setter 通过闭包改动
    // 词法变量，让白盒测试能在两次 saveVisionProvider 之间设定它。
    global.__setVisionNameAutoValue = function (v) { visionNameAutoValue = v; };
  }
