/* IIFE 包裹：把本文件的实现私有化，只有下方显式列出的公共面挂到全局。
...
(function (global) {
  ...
  // 浏览器公共面：仅这些符号可被其它 static/js 文件（主要是 60-settings.js）访问。
  global.THEME_ENGINE_SCHEMA = THEME_ENGINE_SCHEMA;
  global.THEME_DEFAULT_HIGHLIGHT = THEME_DEFAULT_HIGHLIGHT;
  global.THEME_PRESETS = THEME_PRESETS;
  global.THEME_PRESET_MAP = THEME_PRESET_MAP;
  global.THEME_MODE_DEFAULT = THEME_MODE_DEFAULT;
  global.THEME_BUILTIN_CSS_IDS = THEME_BUILTIN_CSS_IDS;
  global.teHexToRgb = teHexToRgb;
  global.teRgbToHex = teRgbToHex;
  global.teSystemPrefersDark = teSystemPrefersDark;
  global.teLookupThemeDef = teLookupThemeDef;
  global.deriveThemeTokens = deriveThemeTokens;
  global.normalizeThemeDef = normalizeThemeDef;
  global.themeDefToExport = themeDefToExport;
  global.resolveActiveThemeId = resolveActiveThemeId;
  global.activeBuiltinFallback = activeBuiltinFallback;
  global.applyAppearance = applyAppearance;
  global.initAppearanceSystemWatch = initAppearanceSystemWatch;
