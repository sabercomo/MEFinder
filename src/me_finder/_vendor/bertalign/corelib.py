...
#   * find_top_k_sents: forced CPU faiss; the upstream GPU branch
#     (``torch.cuda.is_available() and platform == 'linux'``) is removed so the
#     backend never requires CUDA / faiss-gpu. The ``torch`` and ``sys.platform``
#     imports become unused and are dropped.
# Everything else (the two-stage DP, search-path construction, back-tracking,
# similarity/length scoring) is byte-for-byte the upstream algorithm.
import faiss
import numpy as np
import numba as nb
...
def find_top_k_sents(src_vecs, tgt_vecs, k=3):
    ...
    MEFinder modification: CPU-only. Upstream selected a faiss GPU index when
    ``torch.cuda.is_available() and platform == 'linux'``; that branch (and the
    ``torch`` / ``sys.platform`` imports it needed) is removed so the backend
    runs on CPU without CUDA or faiss-gpu. The exact-inner-product result is
    identical to the upstream CPU path.
